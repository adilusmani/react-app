pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("adilusmani/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull adilusmani/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d adilusmani/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
